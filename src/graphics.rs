...
impl RustTowerDefenseApplication {
    ...
    unsafe fn init_hal(window: &Window) -> Result<HalState, &'static str> {
        // Create An Instance
  ...
  > {
    ...
    fn do_the_render(&mut self, local_state: &LocalState) -> Result<(), &'static str> {
        ...
        self.hal_state.draw_triangle_frame(triangle)
    }
    ...
    fn main_loop(&mut self) {
        ...
        loop {
            ...
            if let Err(e) = self.do_the_render(&local_state) {
                ...
                //drop(self.hal_state);
                //self.hal_state = match HalState::new(&self.window_state.window) {
                unsafe {
                    self.hal_state = match RustTowerDefenseApplication::init_hal(&self.window_state.window) {
                        Ok(state) => state,
                        Err(e) => panic!(e),
                    };
                }

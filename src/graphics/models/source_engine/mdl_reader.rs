...
pub struct MDLFileHeader {
	...
	// mstudiobone_t
	pub bone_count: i32,  // Number of data sections (of type mstudiobone_t)
	pub bone_offset: i32, // Offset of first data section

	// mstudiobonecontroller_t
	pub bonecontroller_count: i32,
	pub bonecontroller_offset: i32,
	...
	// mstudioanimdesc_t
	pub localanim_count: i32,
	pub localanim_offset: i32,

	// mstudioseqdesc_t
	pub localseq_count: i32,
	pub localseq_offset: i32,
	...
	pub texturedir_count: i32,
	pub texturedir_offset: i32,

	// Each skin-family assigns a texture-id to a skin location
	skin_reference_count: i32,
	skin_family_count: i32,
	skin_reference_index: i32,

	// mstudiobodyparts_t
	pub bodypart_count: i32,
	pub bodypart_offset: i32,
...
pub fn read_mdl_file_from_disk(path: &str) -> Result<MDLFile, MDLDeserializeError> {
	let mut model_file = match File::open(path) {
		...
		Err(_e) => {
			return Err(MDLDeserializeError::new(
				"Unable to open mdl file from disk",
			));
		}
	};
	...
	if header.id != MDL_HEADER {
		return Err(MDLDeserializeError::new(
			"mdl header not correct; expected [0x49, 0x44, 0x53, 0x54]",
		));
	}
	...
	let name = CStr::from_bytes_with_nul(&header.name[0..first_null])
